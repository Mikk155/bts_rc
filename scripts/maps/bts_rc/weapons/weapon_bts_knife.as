        ...
        private bool Swing( bool fFirst )
        {
            ...
            if( tr.flFraction >= 1.0f )
            {
                if( fFirst )
                {
                    ...
                    self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + ( util::IsTrainedPersonal( m_pPlayer ) ? 0.5f : 0.75f );
            ...
            else
            {
                ...
                self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + ( util::IsTrainedPersonal( m_pPlayer ) ? 0.25f : 0.35f );
                self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
