...
class weapon_bts_screwdriver : ScriptBasePlayerWeaponEntity, bts_rc_base_weapon
{
    ...
    bool Deploy()
    {
        self.DefaultDeploy( self.GetV_Model( "models/bts_rc/weapons/v_screwdriver.mdl" ), self.GetP_Model( "models/bts_rc/weapons/p_screwdriver.mdl" ), DRAW, "crowbar", 0, pev.body );
    ...
    private bool Swing( bool fFirst )
    {
        ...
        if( tr.flFraction >= 1.0f )
        {
            if( fFirst )
            {
                ...
                self.m_flNextPrimaryAttack = g_Engine.time + ( m_fHasHEV ? 0.5f : 0.6f );
        ...
        else
        {
            ...
            self.m_flNextPrimaryAttack = g_Engine.time + ( m_fHasHEV ? 0.25f : 0.35f );
            self.m_flTimeWeaponIdle = g_Engine.time + 2.0f;
