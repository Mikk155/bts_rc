...
void SetupItemTracker()
{
    ...
    CBaseEntity@ ent = g_EntityFuncs.CreateEntity( "game_item_tracker", null, false);

    for(uint ui = 0; ui < arrMenu.length(); ui++)
        g_EntityFuncs.DispatchKeyValue(ent.edict(), arrMenu[ui][0], arrMenu[ui][1]);

    g_EntityFuncs.DispatchSpawn(ent.edict());
    g_Hooks.RegisterHook( Hooks::Player::ClientSay, @PlayerOpenTracker );

    hItemTrackerMenu = EHandle(@ent);
}
...
class CItemCarriers
{
    ...
    {
        this.m_szTargetname = szTargetname;
    ...
    void Find(CBasePlayer@ pRequester)
    {
        ...
        for (int iPlayer = 1; iPlayer <= g_PlayerFuncs.GetNumPlayers(); iPlayer++)
        {
            ...
            while (pInventory !is null)
            {
                ...
                szCarriers.SubString(szCarriers.Length() - 1);
...
class CGameItemTracker : ScriptBaseEntity
{
    ...
    void Spawn()
    {
        ...
        @m_pMenu = CTextMenu(TextMenuPlayerSlotCallback(MenuCallback));
